...
public class AccountControllerTest {
    ...
    public void testAdd() throws Exception {
        ...
        action.andExpect(jsonPath("$.content.joinDate").value(Matchers.is("2018-06-24")));
    }
    ...
    public void testList() throws Exception {
        ...
        action.andExpect(jsonPath("$.data[0].username").value(Matchers.is("11")));
        action.andExpect(jsonPath("$.data[0].email").value(Matchers.is("11@11.com")));
        action.andExpect(jsonPath("$.data[0].forgetPasswordEmail").value(Matchers.is("111@11.com")));
        action.andExpect(jsonPath("$.data[0].phone").value(Matchers.is("11111")));
        action.andExpect(jsonPath("$.data[0].productName").value(Matchers.is("aaaa")));
        action.andExpect(jsonPath("$.data[0].productUrl").value(Matchers.is("http://11.com")));
        action.andExpect(jsonPath("$.data[0].joinDate").value(Matchers.is("2018-05-30")));
    }
